Renderer科学一点讲，是负责管理渲染流程，持有相机视角，调度各种Pass和管理光照、阴影、后处理等的一个代理。玩过Unity的话都比较好理解，我们在URP中修改command buffer就是添加rendererfeature实现的。一个上线的APP添加新的功能叫添加Feature，所以其实还是比较好理解RendererFeature是什么，进而理解虚幻引擎里面的Renderer。
RendererModule.h是长这样的。
